...
def select_display(*display):
    ...
    if len(display) == 0:
        if display_for_sps is not None:
            raise DisplayError('Display already initialized, cannot create new one')
        display_for_sps = pygame.display.set_mode((1600, 900))
...
class nummer:
    ...
    def set(cls, coordinates, width, design):
        ...
        if type(width) is not int and type(width) is not AutoTypeHere:
            raise TypeError(f'The {type(width)} type had passed, but int expected')
        if len(coordinates) != 2:
            raise ValueError(f'The {len(coordinates)} arguments had passed, but 2 ones (x, y) was expected')
        for i in range(2):
        ...
        if type(width) is AutoTypeHere:
            if display_size[0] - coordinates[0] < 15 or display_size[1] - coordinates[1] < 15:
                raise ValueError(f'Width of a nummer cannot be smaller that 15, but coordinated don\'t allowing fitting into the so little space were passed')
            else:
        ...
        if not (type(design).__qualname__.split('.')[0] == 'design'):
            ...
            raise DesignError('You have passed the design, which doesn\'t designed for nummer. Check how you created this')
        cls.exist = True
